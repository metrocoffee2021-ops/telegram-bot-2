...
def add_item(category_id: int, name_uz: str, name_ru: str, name_en: str, has_topping_option: bool) -> int:
    with get_db() as conn:
        cur = conn.execute(
            "INSERT INTO items (category_id, name_uz, name_ru, name_en, has_topping_option, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (category_id, name_uz, name_ru, name_en, int(has_topping_option), db.now_utc().isoformat()),
        )
        return cur.lastrowid
