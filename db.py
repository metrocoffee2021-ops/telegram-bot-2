...
def add_item_to_cart(user_id: int, item_id: int, temp: str, size: str | None, topping: bool, price: int):
    with get_db() as conn:
        conn.execute("INSERT INTO cart (user_id, item_id, temp, size, topping, price) VALUES (?, ?, ?, ?, ?, ?)", (user_id, item_id, temp, size, 1 if topping else 0, price))

def get_user_cart(user_id: int) -> list[dict]:
    with get_db() as conn:
        rows = conn.execute("SELECT cart_id, item_id, temp, size, topping, price FROM cart WHERE user_id = ?", (user_id,)).fetchall()
    return [{"cart_id": r[0], "item_id": r[1], "temp": r[2], "size": r[3], "topping": bool(r[4]), "price": r[5]} for r in rows]
...
def create_order(order_id: str, user_id: int, total: int, payment_method: str):
    with get_db() as conn:
        conn.execute("INSERT INTO orders (order_id, user_id, total, payment_method, created_at) VALUES (?, ?, ?, ?, ?)", (order_id, user_id, total, payment_method, now_utc().isoformat()))
...
def get_order(order_id: str) -> dict | None:
    with get_db() as conn:
        row = conn.execute("SELECT order_id, user_id, total, status, payment_method, gateway_ref FROM orders WHERE order_id = ?", (order_id,)).fetchone()
    if not row: return None
    return {"order_id": row[0], "user_id": row[1], "total": row[2], "status": row[3], "payment_method": row[4], "gateway_ref": row[5]}
