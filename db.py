...
def get_user_order_items(user_id: int) -> list[dict]:
    """All paid orders' items_json for one customer — used to find their most
    frequently ordered item for the 'My usual' quick-reorder button."""
    with get_db() as conn:
        rows = conn.execute(
            "SELECT items_json FROM orders WHERE user_id = ? AND status = 'paid'", (user_id,)
        ).fetchall()
    return [{"items_json": r[0]} for r in rows]


def get_order_count(user_id: int) -> int:
    """Total paid orders for this customer — used for milestone celebrations
    (5th order, 10th order, etc.) in the order confirmation message."""
    with get_db() as conn:
        row = conn.execute(
            "SELECT COUNT(*) FROM orders WHERE user_id = ? AND status = 'paid'", (user_id,)
        ).fetchone()
...
def get_orders_since(iso_datetime: str) -> list[dict]:
    with get_db() as conn:
        rows = conn.execute(
            "SELECT total, items_json, branch_name FROM orders WHERE status = 'paid' AND created_at >= ?",
            (iso_datetime,),
        ).fetchall()
    return [{"total": r[0], "items_json": r[1], "branch_name": r[2]} for r in rows]
...
def ensure_management_tables():
    with get_db() as conn:
        ...
        for col, typ in (("starts_at","TEXT"),("ends_at","TEXT"),("min_subtotal","INTEGER DEFAULT 0"),("max_uses","INTEGER DEFAULT 0"),("used_count","INTEGER DEFAULT 0"),("daily_start","TEXT"),("daily_end","TEXT")):
...
def list_promotions():
    ensure_management_tables()
    with get_db() as c: rows=c.execute("SELECT id,name,code,kind,value,active,created_at,starts_at,ends_at,min_subtotal,max_uses,used_count,daily_start,daily_end FROM promotions ORDER BY id DESC").fetchall()
    return [dict(id=r[0],name=r[1],code=r[2],kind=r[3],value=r[4],active=bool(r[5]),created_at=r[6],starts_at=r[7],ends_at=r[8],min_subtotal=r[9] or 0,max_uses=r[10] or 0,used_count=r[11] or 0,daily_start=r[12],daily_end=r[13]) for r in rows]
def get_promotion(i): return next((p for p in list_promotions() if p['id']==i),None)
def add_promotion(name,code,kind,value,starts_at=None,ends_at=None,min_subtotal=0,max_uses=0,daily_start=None,daily_end=None):
    ensure_management_tables()
    with get_db() as c: cur=c.execute("INSERT INTO promotions(name,code,kind,value,active,created_at,starts_at,ends_at,min_subtotal,max_uses,used_count,daily_start,daily_end) VALUES(?,?,?,?,0,?,?,?,?,?,0,?,?)",(name,code,kind,value,now_utc().isoformat(),starts_at,ends_at,int(min_subtotal),int(max_uses),daily_start,daily_end)); return cur.lastrowid
...
def update_promotion(i,name,code,kind,value,starts_at=None,ends_at=None,min_subtotal=0,max_uses=0,daily_start=None,daily_end=None):
    ensure_management_tables()
    with get_db() as c: c.execute("UPDATE promotions SET name=?,code=?,kind=?,value=?,starts_at=?,ends_at=?,min_subtotal=?,max_uses=?,daily_start=?,daily_end=? WHERE id=?",(name,code,kind,value,starts_at,ends_at,int(min_subtotal),int(max_uses),daily_start,daily_end,i))
